...
describe("Deploying Contracts", function () {
  ...
  let testTwoOwner, testToken, acc1, wAVAXOwner, treasury, acc2, acc3, routerContract, defoTtAddress, factoryAddress, swapTokensToLiquidityThreshold;
  let testTwo, lpManager;
  let table = new Table({
  ...
  beforeEach(async function()  {
    ...
    [testTwoOwner, testTokenOwner, treasury, wAVAXOwner, acc1, acc2, acc3] = await ethers.getSigners();
    ...
    defo = await Defo.deploy(testTwoOwner.address);
    ...
    table.push(
        ...
        ["Defo owner is: ", testTwoOwner.address],
  ...
  it("Defo/TT Lp pair ", async function () {
    table = new Table({
    head:["testTwo/TT Lp Pair's Test", 'Result'],
    colWidths:['auto','auto']
    });
    ...
    await defo.connect(testTwoOwner).transfer(acc1.address, tokenAccOneDecimals);
    const accOneBalanceTestTwo = (await defo.balanceOf(acc1.address)/1e18);
    expect (accOneBalanceTestTwo.toString()).to.equal((tokenAccOneDecimals/1e18).toString()).to.ok;
    ...
    ( testTwo.address, testToken.address,
    ...
    console.log(lpLiquidityBalance)
      table.push(
         ...
         ['Account 1 Defo balance: ', accOneBalanceTestTwo ],
